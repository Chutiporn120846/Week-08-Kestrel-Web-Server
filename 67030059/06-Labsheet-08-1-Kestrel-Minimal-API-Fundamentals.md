...
2. กลับไปที่หน้าเบราว์เซอร์แล้วกดปุ่ม **Refresh (F5)** สังเกตว่าเกิดอะไรขึ้น และอธิบายสั้นๆ ว่าทำไมจึงเป็นเช่นนั้น
   - **คำตอบ** ....................................................................................................
เมื่อกด Ctrl + C โปรแกรม Kestrel Web Server จะหยุดทำงาน ทำให้เว็บเบราว์เซอร์ไม่สามารถเชื่อมต่อกับเว็บไซต์ได้ เมื่อกด Refresh จึงไม่สามารถแสดงหน้าเว็บได้ เนื่องจากไม่มี Web Server ทำงานอยู่ที่ localhost:5173
...
ตัวแปร builder มีหน้าที่เตรียมการตั้งค่าและองค์ประกอบต่าง ๆ ที่จำเป็นสำหรับการสร้าง Web Application เช่น การตั้งค่า Services และ Configuration ส่วนตัวแปร app คือ Web Application ที่สร้างเสร็จแล้วจาก builder ใช้สำหรับกำหนด Route หรือ Endpoint เช่น app.MapGet() และสั่งให้ Web Server เริ่มทำงานด้วย app.Run()
...
.NET Minimal API มีข้อดีคือสามารถสร้าง Web Server และ API ได้อย่างรวดเร็ว ใช้โค้ดน้อย และมี Kestrel Web Server ทำงานร่วมกับแอปพลิเคชันโดยตรง จึงเหมาะกับงาน IoT Gateway ที่ต้องการความรวดเร็วและประสิทธิภาพ ส่วน LAMP Stack ต้องติดตั้งและตั้งค่าหลายส่วน เช่น Apache และ PHP ทำให้มีขั้นตอนมากกว่า แต่มีข้อดีคือเป็นระบบที่นิยม มีเครื่องมือและชุมชนผู้ใช้งานจำนวนมาก
3. นักศึกษาคิดว่าการเพิ่ม `/api/` เข้าไปใน route นั้นมีประโยชน์อย่างไรบ้าง ถ้าไม่ใส่จะเกิดปัญหาอะไรบ้าง
การเพิ่ม /api/ ใน Route ช่วยแยกส่วนของ API ออกจากหน้าเว็บไซต์ทั่วไป ทำให้โครงสร้างของระบบเป็นระเบียบและเข้าใจง่าย เช่น /api/status และ /api/led/on หากไม่ใส่ /api/ ระบบยังสามารถทำงานได้ แต่เมื่อระบบมีขนาดใหญ่ขึ้น อาจทำให้จัดการ Route ได้ยาก สับสนระหว่างหน้าเว็บกับ API และอาจเกิดชื่อ Route ซ้ำกันได้